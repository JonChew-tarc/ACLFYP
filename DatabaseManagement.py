...
class BaseAgentDatabaseManagement:
    ...
    def upload_image(self, resource_id):
        ...
        imageFormatter = "I" + self.imageID


        query_string = "insert into capturedimages (imageID, imageByte) values (%s, %s)"
...
if __name__ == "__main__":
    ...
    #wow = test.read_records("")
    #print (wow)
    dg = DatabaseGateway.DatabaseGateway()
    mydb = dg.db_connect()
    mycursor = mydb.cursor()

    #mycursor.execute("CREATE TABLE capturedimages (imageID VARCHAR(50), imageByte VARCHAR(255)) ENGINE = InnoDB")
